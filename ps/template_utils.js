...
function getSelector(item, parent, sass_indent)
{
	...
	if(item.selectorType == 'classname'){
		...
		selector = layout_id;
		if(sass_indent){
			selector = encodeNameParentRef(selector, parent);
		}
...
function getTemplateData(id)
{
	...
	var path = 'tpl/' + id + '.txt';
	var exportFolder = getDataFolder();
	var output = loadFilePath_cache(exportFolder, path);
	if(output) return{
		output: output,
		type: 'file',
	}
...
function getPlaceHolderValues_rec(parent, config, values)
{
	...
	if(item[OPT_PLACEHOLDER]){
		...
		if(item.type == TYPE_TEXT){
			...
			if(item[OPT_IMGTYPE]=='svg-inline'){
